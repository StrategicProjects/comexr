...
The ComexStat API provides a rich set of auxiliary tables that serve two
purposes: looking up the codes you need for filters, and enriching query
results with human-readable labels.
...
The NCM is the primary product classification for Brazilian trade since
1997. It is an 8-digit code based on the Harmonized System (HS).
...
The HS table provides the international product classification
hierarchy: subheading (6-digit), heading (4-digit), and chapter
(2-digit).
...
**Note:** The ComexStat API does not expose an ISIC table endpoint.
[`comex_isic()`](https://strategicprojects.github.io/comexr/reference/comex_isic.md)
instead retrieves ISIC values from
`/general/filters/ISICSection|ISICDivision|ISICGroup|ISICClass`.

## Using table codes in queries

The main purpose of these tables is to find the numeric codes to use in
`filters`:
...
Some product tables have thousands of entries. Use `page` and `per_page`
to paginate:
...
This is particularly useful when the API is updated with new fields or
metrics.
