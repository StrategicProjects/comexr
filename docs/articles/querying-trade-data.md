...
The comexr package provides three query endpoints, each covering a
...
All query functions return a data.frame (or tibble if the tibble package
...
2.  **Use filters to reduce response size.** The API has a limit of
    ~150,000 rows. If your query is too broad, the API will timeout. Add
